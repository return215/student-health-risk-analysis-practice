...
## Korelasi antara durasi jam kuliah dan durasi jam tidur

```{r class.and.sleep.plot}
class.and.sleep.plot <- new_data |>
  select(Class.Duration, Sleep.Hours, Risk.Level) |>
  ggplot(aes(x = Class.Duration, y = Sleep.Hours)) +
  geom_point(aes(color = Risk.Level)) + # Add points
  labs(
    title = "Correlation Between Class Duration and Sleep Hours",
